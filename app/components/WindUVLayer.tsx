...
export default function WindUvLayer({
  ...
  const trailReadRT = useRef<THREE.WebGLRenderTarget | null>(null);
  const trailWriteRT = useRef<THREE.WebGLRenderTarget | null>(null);
  ...
  useEffect(() => {
    ...
    loader.load(
      ...
      (texture) => {
        ...
        const dimsChanged =
          !uvDimsRef.current ||
          uvDimsRef.current.w !== texW ||
          uvDimsRef.current.h !== texH;
  ...
  mat
...

          {
            ...
            if (!trailReadRT.current) {
              trailReadRT.current = makeTrailRT(texW, texH);
              ...
              renderer.setRenderTarget(trailReadRT.current);
          ...
          {
            // (re)create if missing or size changed
            if (!trailWriteRT.current) {
              trailWriteRT.current = makeTrailRT(texW, texH);
              ...
              renderer.setRenderTarget(trailWriteRT.current);
              renderer.setClearColor(0x000000, 0.0);
              renderer.clear(true, false, false);
          ...
          if (!simSceneRef.current) {
            ...
            simMatRef.current!.uniforms.uSize.value = new THREE.Vector2(
              outW,
              outH
            );
          }
          ...
          const trailStampCam = new THREE.OrthographicCamera(
            -1,
            1,
            1,
            -1,
          ...
          const trailStampPoints = new THREE.Points(
            uvGeoRef.current!,
            trailStampMat
          );
          ...
          const trailOverlayMat = new THREE.ShaderMaterial({
            ...
            blending: THREE.AdditiveBlending,
            uniforms: {
              uTrailTex: { value: trailWriteRT.current!.texture }, // <<— sample the same RT you stamp into
              uOpacity: { value: 0.9 },
          ...
          const trailOverlayMesh = new THREE.Mesh(
            trailOverlayGeom,
            trailOverlayMat
          );
          ...
          apiRef.current = {
            ...
            outW,
            outH,
            trailReadRT: trailReadRT.current!,
          ...
          mat.uniforms.uExaggeration.value =
            typeof exaggeration === "number" ? exaggeration : 0.5;
          mat.uniforms.uAspect.value = aspect;
          mat.uniforms.uPointSize.value =
            1.5 * (window.devicePixelRatio || 1) * 3.0;
          ...
          if (dimsChanged) {
            ...
            mat.uniforms.uSimSize = mat.uniforms.uSimSize || {
            ...
            if (apiRef.current) {
              ...
              apiRef.current.outW = outW;
              apiRef.current.outH = outH;
            }
          } else {
            mat.uniforms.uPrev = mat.uniforms.uPrev || { value: null };
            mat.uniforms.uSimSize = mat.uniforms.uSimSize || {
            ...
            mat.uniforms.uPrev.value = readPositionRTRef.current
              ? readPositionRTRef.current.texture
              : null;
    ...
    return () => {
      ...
      trailReadRT.current?.dispose();
      trailReadRT.current = null;

      trailWriteRT.current?.dispose();
      trailWriteRT.current = null;
